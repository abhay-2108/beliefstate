...
class CircuitBreaker:
    ...
    def record_failure(self) -> None:
        ...
        logger.warning(f"Recorded failure {self.failure_count}/{self.failure_threshold}.")
        if self.failure_count >= self.failure_threshold:
            if self.state != "OPEN":
                logger.error(f"Circuit breaker tripped. State changed to OPEN. Cooldown: {self.recovery_timeout}s.")
            self.state = "OPEN"
            self.last_state_change = time.time()
...
def is_transient_error(exc: Exception) -> bool:
    ...
    if isinstance(exc, (ValueError, TypeError, KeyError, AttributeError, NameError, ImportError)):
        return False
    ...
    if any(k in class_name for k in ["auth", "permission", "invalid", "badrequest", "signature"]):
        return False

    # 7. Check class name for typical retryable issues
    if any(k in class_name for k in ["rate", "limit", "timeout", "connection", "transient", "unavailable", "server", "gateway"]):
        return True
...
class ResilientAdapterWrapper(ProviderAdapter):
    ...
    async def _execute_with_resilience(
        ...
        if self.config.enable_circuit_breaker and not breaker.allow_request():
            logger.error(f"Circuit breaker is OPEN for operation '{op_name}'. Fail fast.")
            raise CircuitBreakerOpenException(f"Circuit breaker is OPEN for '{op_name}'")
