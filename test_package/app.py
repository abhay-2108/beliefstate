        ...
        for entry in st.session_state.conversation:
            ...
            if role == "user":
                ...
                if entry.get("beliefs"):
                    with st.expander(
                        f"📦 {len(entry['beliefs'])} belief(s) extracted",
                        expanded=False,
                    ):
                        for b in entry["beliefs"]:
                            subject = getattr(
                                b,
                                "subject",
                                b.get("subject", "?") if isinstance(b, dict) else "?",
                            )
                            predicate = getattr(
                                b,
                                "predicate",
                                b.get("predicate", "?") if isinstance(b, dict) else "?",
                            )
                            value = getattr(
                                b,
                                "value",
                                b.get("value", "?") if isinstance(b, dict) else "?",
                            )
                            conf = getattr(
                                b,
                                "confidence",
                                b.get("confidence", 0) if isinstance(b, dict) else 0,
                            )
                            st.markdown(
            ...
            with st.spinner(f"Turn {turn}: calling LLM + tracking beliefs..."):
                try:
                    ...
                    async def call_llm(msgs):
                        if provider == "OpenAI":
                            ...
                            client = ollama.AsyncClient(
                                host=f"{ollama_host}:{int(ollama_port)}"
                            )
                    ...
                    llm_response = LLMResponse(
                        text=assistant_text, raw_response=raw_response
                    )
                    ...
                    beliefs = run_async(
                        tracker.get_beliefs(session_id=st.session_state.session_id)
                    )

                    # Check for contradictions by looking at belief changes
                    contradictions = []
                    context_prompt = run_async(
    ...
    with col_live:
        ...
        if st.session_state.conversation:
            last_assistant = next(
                (
                    e
                    for e in reversed(st.session_state.conversation)
                    if e["role"] == "assistant"
                ),
                None,
            )
            ...
            if last_assistant:
                ...
                stages = [
                    ...
                    (
                        "📤 Dispatch",
                        True,
                        f"{'Async bg task' if background_tasks else 'Sync (blocking)'}",
                    ),
...
with tab_tests:
    ...
    async def run_all_tests(tracker, session_id):
        ...
        try:
            ...
            call = LLMCall(
                messages=[{"role": "user", "content": "I love Python."}], kwargs={}
            )
            resp = LLMResponse(text="Got it, you love Python.", raw_response=None)

            await tracker.track_async(
                call.model_dump(), resp.model_dump(), session_id=test_sid, turn=1
            )
            await asyncio.sleep(0.3)

            beliefs = await tracker.get_beliefs(session_id=test_sid)
            assert any(
                "python" in str(getattr(b, "value", "")).lower() for b in beliefs
            )
            results.append(
                {
                    "name": test_name,
        ...
        try:
            ...
            await tracker.track_async(
                call1.model_dump(), resp1.model_dump(), session_id=test_sid, turn=1
            )
            ...
            await tracker.track_async(
                call2.model_dump(), resp2.model_dump(), session_id=test_sid, turn=2
            )
            await asyncio.sleep(0.5)
        ...
        try:
            ...
            call_a = LLMCall(
                messages=[{"role": "user", "content": "I am Alice."}], kwargs={}
            )
            resp_a = LLMResponse(text="Hello Alice.", raw_response=None)
            await tracker.track_async(
                call_a.model_dump(), resp_a.model_dump(), session_id=sid_a, turn=1
            )

            call_b = LLMCall(
                messages=[{"role": "user", "content": "I am Bob."}], kwargs={}
            )
            resp_b = LLMResponse(text="Hello Bob.", raw_response=None)
            await tracker.track_async(
                call_b.model_dump(), resp_b.model_dump(), session_id=sid_b, turn=1
            )
        ...
        try:
            dispatcher_type = (
                tracker._config.task_dispatcher_type
                if hasattr(tracker, "_config")
                else "unknown"
            )
            results.append(
                {
...
with tab_context:
    ...
    with col_ctx1:
        ctx_session = st.text_input(
            "Session ID", value=st.session_state.session_id, key="ctx_sid"
        )
